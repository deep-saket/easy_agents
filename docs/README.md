# Documentation

`easy_agents` is an early-stage, graph-native framework and collection of concrete agents. Start with the path that matches what you want to do.

## Start Here

- **Run something locally:** [Getting Started](./getting-started.md)
- **Understand what works today:** [Current Capabilities](./reference/current-capabilities.md)
- **Inspect one tested capability:** [Functionality Catalog and Test Matrix](./reference/functionalities/README.md)
- **Inspect the shared foundation:** [Commons Circle Runtime](./reference/functionalities/commons.md)
- **See what remains for Commons:** [Commons Completion Plan](./plans/commons-completion-plan.md)
- **See the model-planned Chat implementation:** [Natural-Language Chat Completion Plan](./plans/natural-language-chat-completion-plan.md)
- **Build a new agent:** [Create an Agent](./guides/create-an-agent.md)
- **Build a complete Galaxy:** [Build Your Own Galaxy](./guides/build-your-galaxy.md)
- **Chat with the system:** [Chat with Your Galaxy](./guides/chat-with-your-galaxy.md)
- **Design the personal agent system:** [Personal Agent Constellation](./vision/personal-agent-constellation.md)
- **Use the shared system vocabulary:** [Galaxy Terminology](./architecture/galaxy-terminology.md)
- **Migrate code to that vocabulary:** [Terminology-First Domain Model Migration](./plans/terminology-domain-model-migration.md)
- **Classify a new feature:** [Feature Intake and Draft Charters](./guides/feature-intake.md)
- **Use the Mac-hosted model:** [Local Gemma Integration](./guides/use-local-gemma.md)
- **Explore agents and components:** [Galaxy Knowledge Graph](./guides/constellation-map.md)
- **Run any Specialist:** [Complete Specialist Fleet](./guides/run-specialist-fleet.md)
- **Plan live fleet operations:** [Realtime Monitoring and Tracing Plan](./plans/realtime-monitoring-and-tracing-plan.md)
- **Plan the employed deep-tech explorer system:** [Personal and Deep-Tech Explorer Implementation Plan](./plans/deep-tech-explorer-implementation-plan.md)
- **Understand the target platform:** [Offline Agent Platform Roadmap](./plans/offline-agent-platform-roadmap.md)

The capability reference describes implemented behavior. The roadmap describes future work, including manifest-driven agents and a platform CLI that do not exist yet.

## Architecture

- [Galaxy Terminology](./architecture/galaxy-terminology.md)
- [ADR 0001: Canonical Galaxy Vocabulary](./architecture/decisions/0001-canonical-galaxy-vocabulary.md)
- [Framework Overview](./architecture/framework-overview.md)
- [Conversation Management Architecture](./architecture/conversation-management.md)
- [Memory Architecture](./architecture/memory-architecture.md)

## Function Reference

- [Agent Runtime and Reusable Nodes](./reference/functionalities/agent-runtime-and-nodes.md)
- [Satellites (Tools) and Execution](./reference/functionalities/tools.md)
- [Memory and Retrieval](./reference/functionalities/memory.md)
- [Model Adapters](./reference/functionalities/models.md)
- [Channels, Sources, and Voice](./reference/functionalities/channels-and-voice.md)
- [Observability](./reference/functionalities/observability.md)
- [Graph Builder](./reference/functionalities/graph-builder.md)
- [Configuration](./reference/functionalities/configuration.md)
- [Concrete Agents](./reference/functionalities/agents.md)
- [Constellation Feature Intake](./reference/functionalities/constellation-feature-intake.md)
- [Canonical Galaxy Domain](./reference/functionalities/galaxy-domain.md)
- [Commons Circle Runtime](./reference/functionalities/commons.md)
- [Galaxy Domain API](./reference/galaxy-domain-api.md)

## Working and Experimental Agents

- [Collection Agent](../agents/collection_agent/README.md) — the most complete domain workflow
- [Discount Planning Agent](../agents/discount_planning_agent/README.md) — collection negotiation specialist
- [Collection Memory Helper](../agents/collection_memory_helper_agent/README.md) — collection key-event memory specialist
- [MailMind Overview](./agents/mailmind/overview.md)
- [MailMind Scope](./agents/mailmind/scope.md)
- [MailMind Scope Mapping](./agents/mailmind/scope-mapping.md)
- [MailMind Graph Spec](./agents/mailmind/graph.md)
- [Simple Conversation Agent](./agents/simple-conversation.md)
- [Conversation Manager Agent](./agents/conversation-manager.md)

## Placeholder Agents

These pages reserve planned roles; they do not describe complete runnable agents:

- [Brainstorming Agent](./agents/brainstorming-agent.md)
- [Coding Agent](./agents/coding-agent.md)
- [Orchestrator Agent](./agents/orchestrator-agent.md)

## Plans and Conventions

- [Documentation Plan](./plans/documentation-plan.md)
- [Offline Agent Platform Roadmap](./plans/offline-agent-platform-roadmap.md)
- [Personal Constellation Gap and Delivery Plan](./plans/personal-constellation-plan.md)
- [Personal and Deep-Tech Explorer Implementation Plan](./plans/deep-tech-explorer-implementation-plan.md)
- [Realtime Monitoring and Tracing Plan](./plans/realtime-monitoring-and-tracing-plan.md)
- [Terminology-First Domain Model Migration](./plans/terminology-domain-model-migration.md)
- [Memory Vector Index Plan](./plans/memory-vector-index-plan.md)
- [Commons Completion Plan](./plans/commons-completion-plan.md)
- [Natural-Language Chat Completion Plan](./plans/natural-language-chat-completion-plan.md)
- [Refactor Fix Plan](./plans/refactor-fix-plan.md)
- [Docstring Rules](./conventions/docstring-rules.md)
