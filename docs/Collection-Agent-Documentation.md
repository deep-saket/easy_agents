...
**Work period covered:** 18 May 2026 to 18 July 2026  
...
| Current local work | Uncommitted at review time | Objective-driven response validation and runtime plan projection |
...
| KPI | Definition | Example calculation |
|---|---|---|
| Intent Accuracy | Correct classification of customer intent/posture | correct intent labels / labeled turns |
| Workflow Completion Rate | Conversations reaching expected terminal outcome | completed expected outcomes / eligible sessions |
| Tool Success Rate | Required tools completing successfully | successful required calls / required calls |
| Tool Selection Accuracy | Correct tool chosen when action required | correct tool decisions / tool-decision turns |
| Tool Argument Accuracy | Required arguments exactly/semantically correct | valid calls / selected calls |
| Policy Compliance | Turns satisfying all deterministic policy invariants | compliant turns / evaluated turns |
| Response Quality | Rubric score for clarity, empathy, professionalism, non-repetition | human or calibrated judge score |
| Latency | Per-turn and per-node response time | median, P95, P99 |
| Hallucination Rate | Responses containing unsupported consequential facts | hallucinated responses / responses |
| State Transition Accuracy | Expected state diffs correctly produced | correct transitions / labeled transitions |
| Node Accuracy | Correct graph route/node sequence | correct route decisions / route decisions |
| Conversation Completion Rate | Sessions that close/transfer without deadlock | terminal sessions / started sessions |
| Privacy Compliance | No unauthorized disclosure or verification bypass | compliant privacy cases / privacy cases |
| Wrong-Party Safety | Wrong-party cases with zero account disclosure | safe cases / wrong-party cases |
| Partial Payment Success | Valid partial workflows completed correctly | completed valid partial cases / valid partial cases |
| Discount Workflow Accuracy | Correct eligibility, amount, confirmation, escalation | correct discount sessions / discount sessions |
| Callback Scheduling Accuracy | Correct normalized time and durable job | correct scheduled callbacks / callback requests |
| Repetition Rate | Unnecessary repeated agent objectives/responses | repeated turns / agent turns |
| Reflection Recovery Rate | Failed plans corrected within retry budget | recovered failures / reflection-triggered failures |

Metrics should be segmented by workflow, customer language, policy, model, and failure type. A high aggregate score can hide a serious wrong-party privacy regression.
...
- Evaluate dynamic objective trees against event logs.
- Study calibrated model graders for empathy and non-repetition.
- Develop privacy-focused adversarial conversation generation.
- Measure robustness to ASR errors and multilingual/code-switched speech.
- Explore process evaluation using structured decisions without exposing private chain-of-thought.
- Investigate causal attribution of failures across classifier, planner, tool, state, and renderer.
- Build offline-to-online evaluation links using anonymized production telemetry.
- Study confidence-aware routing to human specialists.
