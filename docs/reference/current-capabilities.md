...
| Wormhole Chat | Control Room Chat mode, persistent bounded conversation IDs, explicit durable memory, canonical Trajectory, four executable local Satellites, validated tool evidence, Map highlighting, Gemma answers, readiness reporting, and Mission/Satellite observability | Functional local advisory chat; Calculate, Unit Convert, Memory Search, and Memory Write execute through shared authorization; network and external-send Satellites remain disabled, and there is no authenticated non-loopback deployment | [Chat with Your Galaxy](../guides/chat-with-your-galaxy.md) |
| Commons Circle | Drift-checked readiness for all 30 shared non-Planet components, answer-source provenance, model completion/token evidence, basic output-quality checks, routing, policy, memory, observability, and four local Satellites | 12 components are operational, 10 partial, 4 declared-only, and 4 disabled; syntactic generation checks do not prove semantic correctness | [Commons Circle runtime](./functionalities/commons.md) |
