...
| Canonical Galaxy domain | Immutable Galaxy, Circle, Rocky/Giant Planet, Component, Satellite, Constellation, Solar System, Wormhole, Rogue Star, Mission, policy, registry, YAML v2, topology, CLI, and v2 API contracts | Core composition, validation, deterministic routing, synchronous handler execution, Satellite authorization, and all-70-role v1 adaptation are implemented; durable cancellation, complete budget accounting, event aliases, authentication, and legacy-application migration remain planned | [Galaxy domain](./functionalities/galaxy-domain.md) |
| Wormhole Chat | Gemma-selected Circle, Planet, and typed multi-action plan; 18 advisory/Satellite/Commons actions; persistent bounded conversations; named-Vault selection; canonical Trajectory; verified local results; Map highlighting; readiness; and correlated planning/generation/action observability | Functional local Chat with no keyword/regex intent router and no deterministic intent fallback; invalid plans retry only through Gemma; network and external-send actions remain absent, and there is no authenticated non-loopback deployment | [Chat with Your Galaxy](../guides/chat-with-your-galaxy.md) |
| Commons Circle | One shared SQLite runtime and Control Room workspace for named Vaults, immutable artifact versions, local source/citation search, review scheduling, approval records, calendar proposals, response provenance, routing, policy, observability, and local Satellites | 23 components are operational, 3 partial, 0 declared-only, and 4 deliberately disabled; lexical evidence scores and syntactic generation checks do not prove semantic correctness | [Commons Circle runtime](./functionalities/commons.md) |
