...
| Wormhole Chat | Control Room Chat mode, conversation IDs, bounded ephemeral turn context, canonical Trajectory, Constellation/Satellite context, Map highlighting, Gemma answer, and Mission observability | Functional local read-only chat; model reasoning only, no automatic Satellite invocation, no durable Vault history, and no authenticated non-loopback deployment | [Chat with Your Galaxy](../guides/chat-with-your-galaxy.md) |
