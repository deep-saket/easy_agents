...
Purpose: Implements the DuckDB repository for MailMind persistence.
"""
...
class DuckDBMessageRepository(MessageRepository):
    """Stores MailMind operational data in DuckDB."""
        ...
        for row in rows:
            bundles.append(
                MessageBundle(
                    ...
                    classification=ClassificationResult.model_validate_json(row["classification_json"]) if row.get("classification_json") else None,
    ...
    def list_conversation_messages(self, session_id: str, limit: int = 100) -> list[ConversationMessage]:
        rows = self._fetchall(
            """
