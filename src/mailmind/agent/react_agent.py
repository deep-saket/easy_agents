...
from mailmind.schemas.tools import PlannerDecision, ToolExecutionResult
...
class ReActAgent:
    ...
    def run(self, user_input: str, session_id: str) -> str:
        memory = ConversationMemory.load(session_id, self.repository)
    ...
    def _route_after_reason(self, state: ReActState) -> str:
        ...
        if decision.respond_directly or decision.done:
            return "respond"
    ...
    def _act_node(self, state: ReActState) -> ReActState:
        ...
        tool_result = ToolExecutionResult.model_validate(
            self.executor.execute(decision.tool_call.tool_name, decision.tool_call.arguments)
        )
    ...
    def _respond_node(self, state: ReActState) -> ReActState:
        decision = state["decision"]
        response = decision.response_text or state.get("response") or "Done."
