...
const elements = {
  ...
  runtimeCommonsReadiness: document.getElementById("runtime-commons-readiness"),
...
async function loadGemmaStatus() {
  ...
  try {
    ...
    const commons = readiness.commons?.summary || {};
    elements.runtimeCommonsReadiness.textContent = `Commons: ${commons.operational || 0} operational · ${commons.partial || 0} partial · ${commons.declared || 0} declared · ${commons.disabled || 0} disabled.`;
    ...
    elements.runtimeCommonsReadiness.textContent = "Could not load the Commons implementation audit.";
  }
...
function appendChatMessage(role, content, payload = null, extraClass = "") {
  ...
  if (payload) {
    ...
    for (const label of [
      payload.status,
      planet?.display_name,
      payload.answer_source ? `${payload.answer_source} answer` : null,
      payload.generation
        ? `${payload.generation.model_id} · ${payload.generation.output_used ? "used" : "not used"}`
        : payload.used_model,
...
function generationEvidenceSummary(generation) {
  ...
  const details = [
    generation.completed ? "request completed" : "request failed",
    `quality ${generation.quality_status}`,
    generation.finish_reason ? `finish ${generation.finish_reason}` : null,
    Number.isFinite(generation.total_tokens) ? `${generation.total_tokens} tokens` : "token usage unavailable",
    Number.isFinite(generation.duration_ms) ? `${Math.round(generation.duration_ms)} ms` : null,
    generation.output_used ? "model output used" : "model output not used",
  ].filter(Boolean);
  if (generation.quality_checks?.length) {
    details.push(`checks: ${generation.quality_checks.join(", ")}`);
  }
  return details.join(" · ");
}
...
function renderChatRoute(payload) {
  ...
  if (context.available_satellites?.length) {
    ...
    invocation.textContent = context.invoked_satellites?.length
      ...
      : payload.answer_source === "model"
        ? "No Satellite was invoked; model-generated text was used for this answer."
        : "No Satellite was invoked for this answer.";
  ...
  provenance.append(
    chatRouteStep(
      "✓",
      sourceLabels[payload.answer_source] || "Unknown answer source",
      generationEvidenceSummary(payload.generation),
      "provenance",
    ),
  );
  elements.chatRouteContext.append(provenance);
